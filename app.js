...
const S = {
  ...
  catalog:{categories:[],products:[]}, cart:[], cat:null, orders:[],
...
/* ─── الدخول ─── */
// Guarded: a missing node must never blank the whole page
...
function lazySub(key, build){
  ...
  _lazy[key]=onSnapshot(build(), snap=>{
    ...
    if(key==='orders2')S.orders=snap.docs.map(d=>({id:d.id,...d.data()}));
    render();
  },()=>{});
}
...
function ensurePageData(){
  ...
  if(S.page==='tx'){
    ...
    lazySub('orders2',()=>query(collection(db,'orders'),
      where('uid','==',uid),orderBy('created_at','desc'),limit(40)));
    dropLazy('orders2');
...
/* ─── المتجر ─── */
const lyd2=v=>Number(v||0).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+' د.ل';
const cartCount=()=>S.cart.reduce((t,i)=>t+i.qty,0);
const cartTotal=()=>S.cart.reduce((t,i)=>t+i.price_lyd*i.qty,0);

function loadCatalog(){
  if(S.catalog.products.length)return;
...
function vShop(){
  ...
  if(!cats.length)return `
    <div class="h1" style="margin-bottom:18px">الأقسام</div>
    <div class="panel"><div class="empty">
      <div class="ico">${svg(I.grid,1.6)}</div>
      <p style="font-size:13.5px">لا أقسام بعد.</p></div></div>`;

  return `
  <div class="h1" style="margin-bottom:5px">الأقسام</div>
  <p class="sub" style="margin-bottom:20px">اختر القسم لتصفّح منتجاته.</p>
  <div class="catgrid">
    ${cats.map((c,i)=>`
      <button class="cattile ${c.soon?'soon':''}" data-cat="${esc(c.id)}"
              style="animation-delay:${i*.05}s">
        ${c.soon?'<span class="soonbadge">قريبًا</span>':''}
        <span class="catico">${c.image
          ? `<img src="${esc(c.image)}" alt="" loading="lazy">`
          : svg(I.grid,1.7)}</span>
        <span class="catname">${esc(c.name)}</span>
        <span class="catcount">${
          c.soon?'قيد التجهيز'
               :S.catalog.products.filter(p=>p.cat===c.id).length+' منتج'}</span>
      </button>`).join('')}
  </div>`;
}
...
function vCat(){
  ...
  if(!c)return vShop();
  const items=S.catalog.products.filter(p=>p.cat===c.id);

  return `
  <div style="display:flex;align-items:center;gap:11px;margin-bottom:6px">
    <button class="ibtn" data-back="shop">${svg('<path d="M9 6l6 6-6 6"/>',2.2)}</button>
    <div class="h1" style="margin:0">${esc(c.name)}</div>
  </div>
  <p class="sub" style="margin-bottom:18px">${items.length} منتج متاح.</p>

  ${items.length?`<div class="prodgrid">
    ${items.map((p,i)=>`
      <button class="prod ${p.available?'':'out'}" data-prod="${esc(p.id)}"
              style="animation-delay:${i*.04}s">
        <span class="prodimg">${p.image
          ? `<img src="${esc(p.image)}" alt="" loading="lazy">`
          : svg(I.bag,1.6)}</span>
        <span class="prodbody">
          <span class="prodname">${esc(p.name)}</span>
          <span class="prodmeta">${p.kind==='stock'?'تسليم فوري':'تنفيذ يدوي'}</span>
          <span class="prodprice num">${lyd2(p.price)}</span>
        </span>
        ${p.available?`<span class="prodadd">${svg(I.plus2,2.2)}</span>`
                     :`<span class="chip bad">نفد</span>`}
      </button>`).join('')}
  </div>`
  :`<div class="panel"><div class="empty">
      <div class="ico">${svg(I.bag,1.6)}</div>
      <p style="font-size:13.5px">لا منتجات في هذا القسم بعد.</p></div></div>`}`;
}
...
function productSheet(id){
  ...
  sheet(`
    <div class="prodhead">
      <div class="prodhead-img">${p.image
        ? `<img src="${esc(p.image)}" alt="">`
        : svg(I.bag,1.5)}</div>
      <div style="min-width:0">
        <div class="h2">${esc(p.name)}</div>
        <div class="num" style="color:var(--accent);font-size:19px;margin-top:2px">
          ${lyd2(p.price)}</div>
    ...
    ${p.desc?`<div class="panel-q" style="margin-bottom:14px">
      <p class="sub" style="font-size:13px;line-height:1.7;white-space:pre-line"
         >${esc(p.desc)}</p></div>`:''}

    ${p.fields.map(f=>`
      <label class="lbl" style="margin-top:12px">${esc(f.label)}${
        f.required?'':' <span style="color:var(--text-3);font-weight:400">(اختياري)</span>'}</label>
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-top:18px">
      <div class="qty">
        <button class="qbtn" data-q="-">−</button>
        <span class="num" id="pQty">1</span>
        <button class="qbtn" data-q="+">+</button>
      </div>
      <div style="flex:1;text-align:start">
        <div class="payk">الإجمالي</div>
        <div class="num" id="pTot" style="font-size:18px">${lyd2(p.price)}</div>
      </div>
    </div>

    ${p.note?`<div class="note" style="margin-top:14px">${esc(p.note)}</div>`:''}

    <button class="btn wide" id="pAdd" style="margin-top:16px"
            ${p.available?'':'disabled'}>
      ${p.available?'إضافة إلى السلة':'غير متوفر'}</button>
    <button class="btn line wide" style="margin-top:9px" onclick="closeSheet()">إلغاء</button>`);

  let qty=1;
  const paint=()=>{
    $('#pQty').textContent=qty;
    $('#pTot').textContent=lyd2(p.price*qty);
  };
  ...
  $('#pAdd').onclick=()=>{
    ...
    S.cart.push({id:p.id,name:p.name,image:p.image,price_lyd:p.price,qty,values:vals,kind:p.kind});
    closeSheet();
...
function cartSheet(){
  if(!S.cart.length)return sheet(`
    ...
    <div class="empty"><div class="ico">${svg(I.cart,1.6)}</div>
      <p style="font-size:13.5px">سلتك فارغة.</p></div>
    <button class="btn line wide" onclick="closeSheet()">حسنًا</button>`);

  const tot=cartTotal();
  const totUsd=tot/((S.config.usd_to_lyd)||11.8);
  const enough=(S.profile.wallet_balance||0)>=totUsd;

  sheet(`
    <div class="h2" style="margin-bottom:14px">السلة (${cartCount()})</div>
    <div class="feed" style="margin-bottom:14px">
      ${S.cart.map((it,i)=>`
        <div class="fitem">
          <div class="fico">${it.image
            ? `<img src="${esc(it.image)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:11px">`
            : svg(I.bag)}</div>
          <div class="fmain">
            <div class="ftitle">${esc(it.name)}</div>
            <div class="fmeta">${it.qty} × ${lyd2(it.price_lyd)}</div>
          </div>
          <div style="display:flex;align-items:center;gap:9px">
            <div class="num" style="font-size:14px">${lyd2(it.price_lyd*it.qty)}</div>
            <button class="ibtn" data-rm="${i}" aria-label="حذف">${svg(I.trash2)}</button>
          </div>
        </div>`).join('')}
    </div>
    ...
    <div class="quote">
      ...
      <div class="qrow tot"><span>الإجمالي</span>
        <span class="num">${lyd2(tot)}</span></div>
    </div>

    ${enough?'':`<div class="note" style="margin-top:12px">
      رصيدك ${usd(S.profile.wallet_balance)} — غير كافٍ.
      <button style="color:var(--accent);font-weight:700"
              onclick="closeSheet();depositSheet()">إضافة رصيد</button></div>`}

    <button class="btn wide" id="cPay" style="margin-top:16px" ${enough?'':'disabled'}>
      تأكيد الشراء</button>
  ...
  $('#cPay').onclick=async()=>{
    ...
    try{
      ...
      toast(e.message,'bad');
      b.disabled=false;b.textContent='تأكيد الشراء';
    }
  };
}
...
function orderDone(o){
  ...
  sheet(`
    <div style="text-align:center;margin-bottom:20px">
      <div style="width:62px;height:62px;border-radius:19px;margin:0 auto 15px;
           display:grid;place-items:center;
           background:${o.status==='completed'?'var(--accent-soft)':'rgba(224,160,48,.14)'};
           color:${o.status==='completed'?'var(--accent)':'var(--warn)'}">
    ...
    ${codes.length?`<div class="paybox">
      ${codes.map(i=>i.codes.map(c=>`
        <div class="payrow">
          <div style="min-width:0"><div class="payk">${esc(i.name)}</div>
            <div class="payv mono" dir="ltr">${esc(c)}</div></div>
          <button class="ibtn" data-cpc="${esc(c)}">${svg(I.copy)}</button>
        </div>`).join('')).join('')}
    </div>`:''}
  ...
  document.querySelectorAll('[data-cpc]').forEach(b=>
    b.onclick=()=>copy(b.dataset.cpc,'الكود'));
}

const OST={completed:['ok','مكتملة'],pending:['wait','قيد التنفيذ'],
  processing:['wait','قيد المعالجة'],rejected:['bad','مرفوضة']};

function vOrders(){
  const rows=S.orders;
  return `
  <div class="h1" style="margin-bottom:5px">طلباتي</div>
  <p class="sub" style="margin-bottom:18px">حالة كل طلب ومنتجاته.</p>
  ${rows.length?`<div class="feed">${rows.map(o=>{
    const st=OST[o.status]||['off','—'];
    const n=(o.items||[]).length;
    return `<div class="fitem" data-order="${esc(o.id)}">
      <div class="fico">${svg(I.bag)}</div>
      <div class="fmain">
        <div class="ftitle">${esc((o.items&&o.items[0]&&o.items[0].name)||'طلب')}${
          n>1?` +${n-1}`:''}</div>
        <div class="fmeta">${dt(o.created_at)}</div>
      </div>
      <div class="fval">
        <div class="famt num">${lyd2(o.total_lyd)}</div>
        <span class="chip ${st[0]}" style="margin-top:3px">${st[1]}</span>
      </div></div>`;}).join('')}</div>`
   :`<div class="panel"><div class="empty">
      <div class="ico">${svg(I.bag,1.6)}</div>
      <div style="font-weight:600;color:var(--text);margin-bottom:5px">لا طلبات بعد</div>
      <p style="font-size:13px;margin-bottom:18px">تصفّح الأقسام وابدأ أول طلب.</p>
...
function orderSheet(id){
  ...
  sheet(`
    <div style="display:flex;justify-content:space-between;align-items:flex-start;
      ...
      <div><div class="h2">طلب ${esc(id.slice(-8))}</div>
        <div class="fmeta">${dt(o.created_at)}</div></div>
      <span class="chip ${st[0]}">${st[1]}</span>
    </div>
    ...
    <div class="feed" style="margin-bottom:14px">
      ${(o.items||[]).map(i=>`
        <div class="fitem">
          <div class="fico">${i.image
            ? `<img src="${esc(i.image)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:11px">`
            : svg(I.bag)}</div>
          <div class="fmain">
            <div class="ftitle">${esc(i.name)}</div>
            <div class="fmeta">${i.qty} × ${lyd2(i.price_lyd)}</div>
            ${i.values&&Object.keys(i.values).length?`<div class="fmeta mono" dir="ltr"
              style="opacity:.8">${Object.values(i.values).map(esc).join(' · ')}</div>`:''}
          </div>
          <div class="num" style="font-size:13.5px">${lyd2(i.line_lyd)}</div>
        </div>`).join('')}
    </div>
    ...
    ${o.delivery?`<div class="lbl" style="margin-top:6px">ملاحظة التنفيذ</div>
      <div class="panel-q"><p class="sub" style="font-size:13px;white-space:pre-line"
        >${esc(o.delivery)}</p></div>`:''}

    ${o.reject_reason?`<div class="note" style="margin-top:12px">
      سبب الرفض: ${esc(o.reject_reason)} — أُعيد المبلغ إلى محفظتك.</div>`:''}

    <div class="quote" style="margin-top:14px">
      <div class="qrow tot"><span>الإجمالي</span>
        <span class="num">${lyd2(o.total_lyd)}</span></div>
    </div>
...
function bind(){
  ...
  document.querySelectorAll('[data-cat]').forEach(b=>
    b.onclick=()=>{
      const c=S.catalog.categories.find(x=>x.id===b.dataset.cat);
      if(c&&c.soon)return soonSheet(c);
      S.cat=b.dataset.cat; S.dir='fwd'; render();
    });
  document.querySelectorAll('[data-prod]').forEach(b=>
    b.onclick=()=>productSheet(b.dataset.prod));
  document.querySelectorAll('[data-back]').forEach(b=>
    b.onclick=()=>{ S.cat=null; S.dir='back'; render(); });
