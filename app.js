...
function openServiceOrderDetail(orderId) {
  ...
  modal(`
    <div class="modal-head">
      ...
      <button class="modal-close" onclick="closeModal()" aria-label="إغلاق">${svg(I.x, 2)}</button>
    </div>
  ...
  onTap('#svcBuyBtn', async () => {
    ...
    try {
      const r = await api('/api/service/create-order', {
        service_id: svcId,
        inputs,
        idempotency_key: newIdemKey(),  // ⭐ Idempotency
      });
  ...
  <div class="page-enter">
    ...
    <div class="card mb-5">
      ...
      <div class="field mb-4">
        ...
        <input class="input" id="pPhone" value="${esc(S.profile.phone || '')}" placeholder="0912345678" dir="ltr">
      </div>
...
function bind() {
  ...
  onTap('#doLogout', () => {
    modal(`
      ...
      <button class="btn btn-secondary btn-block" style="margin-top:8px" onclick="closeModal()">إلغاء</button>
    `);
...
async function openDeposit() {
  modal(`
    ...
    <button class="btn btn-secondary btn-block" style="margin-top:16px" onclick="closeModal()">إلغاء</button>
  `);
  $$('[data-method]').forEach(b => b.onclick = () => openDepositMethod(b.dataset.method));
}
...
async function openDepositMethod(method) {
  ...
  modal(`
    ...
    ${phone ? `
      <div class="card card-sm mb-4">
        ...
        <div class="copy-box">
          ...
          <button class="copy-btn" onclick="navigator.clipboard.writeText('${esc(phone)}').then(()=>window.__toast('نُسخ','ok'))" aria-label="نسخ">${svg(I.copy, 2)}</button>
        </div>
      </div>
    ` : ''}
    ...
    <div class="field mb-3">
      ...
      <input class="input" id="dpPhone" dir="ltr" inputmode="numeric"
             placeholder="912345678" maxlength="13" value="${esc(S.profile.phone || '')}">
    </div>
  ...
  $('#dpSubmit').onclick = async () => {
    const btn = $('#dpSubmit');
    const phoneVal = $('#dpPhone').value.trim();
    const amount = n($('#dpAmount').value);

    if (phoneVal.replace(/\D/g, '').length < 9) return toast('اكتب رقمك كاملاً', 'bad');
    if (!(amount > 0)) return toast('أدخل المبلغ', 'bad');

    btn.disabled = true;
    btn.classList.add('loading');
    try {
      const r = await api('/api/wallet/claim', { phone: phoneVal, amount_lyd: amount, method });
      closeModal();
  ...
  onTap('#revealBtn', async () => {
    ...
    try {
      ...
      modal(`
        ...
        <div class="alert alert-warning mb-4">${svg(I.help, 2)}<div>ستختفي هذه البيانات خلال ${hrs} ساعة. احفظها الآن.</div></div>
        ...
        <div class="grid-2 mb-4">
          ...
          <div>
            ...
            <div class="copy-box">
              ...
              <button class="copy-btn" data-copy="${esc(d.cvv)}" aria-label="نسخ">${svg(I.copy, 2)}</button>
            </div>
          </div>
        </div>
