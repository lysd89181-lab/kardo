...
function buildNav(){
  ...
  $('#navDock').innerHTML=dk.map(k=>{
    if(k==='more')return `<button class="dk" data-nav="more">${svg(I.more)}<span>المزيد</span></button>`;
    const p=PAGES.find(x=>x.k===k);
    return `<button class="dk" data-nav="${p.k}">${svg(p.i)}<span>${p.t}</span></button>`;
  }).join('');
...
function moreSheet(){
  ...
  sheet(`<div class="h2" style="margin-bottom:14px">المزيد</div>`+
    ['tx','help','settings'].filter(k=>keys.includes(k)).map(k=>{const p=PAGES.find(x=>x.k===k);
      return `<button class="btn line wide" style="margin-bottom:9px;justify-content:flex-start"
        onclick="closeSheet();go('${p.k}')">${svg(p.i)} ${p.t}</button>`;}).join('')+
    `<button class="btn line wide" style="justify-content:flex-start"
       onclick="closeSheet();askOut()">تسجيل الخروج</button>`);
}
...
function vHome(){
  ...
  <div class="hero">
    ...
    <div class="bal num" id="heroBal">${usd(S.profile.wallet_balance)}</div>
    <div class="bal-lyd">≈ ${lyd(S.profile.wallet_balance)}</div>
    <div class="hero-acts">
      <button class="btn" data-act="deposit">${svg(I.up,2)} إضافة رصيد</button>
      <button class="btn line" data-act="issue">${svg(I.plus)} إصدار بطاقة</button>
    </div>
  </div>
  ...
  ${S.cards.length?`
    <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:13px">
      ...
      <button data-act="cards" style="color:var(--accent);font-weight:600;font-size:13px">
        عرض الكل</button>
    </div>
      ...
      <div class="fmain">
        <div class="ftitle num">${usd(d.amount_usd)}
          <span style="font-weight:400;color:var(--text-3);font-size:12px;
                font-family:'IBM Plex Sans Arabic'"> · ${esc(d.method||'—')}</span></div>
...
function depositSheet(){
  ...
  const all=['libyana','almadar','usdt','bank','binance'];
  ...
  sheet(`
    ...
    <p class="sub" style="margin-bottom:18px">اختر طريقة التحويل التي تناسبك.</p>
    <div class="mgrid">
      ${list.map((m,i)=>`
  ...
  <div class="feed" style="margin-bottom:26px">
    ${steps.map(([t,d],i)=>`<div class="fitem">
      <div class="fico" style="background:var(--accent-soft);color:var(--accent);
           font-weight:700;font-family:'IBM Plex Mono'">${i+1}</div>
