...
async function handleStatus(env) {
  ...
  return {
    ...
    nav: {
      ...
      tx: s.nav_tx !== false, settings: s.nav_settings !== false,
    },
    texts: {
    ...
    methods: {
      libyana: { on: s.m_libyana_on !== false, label: s.m_libyana_label || 'ليبيانا',
                 rate: num(s.rate_libyana, 11.8), auto: true },
      almadar: { on: s.m_almadar_on !== false, label: s.m_almadar_label || 'المدار',
                 rate: num(s.rate_libyana, 11.8), auto: true },
      bank:    { on: s.m_bank_on === true, label: s.m_bank_label || 'تحويل مصرفي',
                 rate: num(s.rate_bank, 9.5), auto: false, details: s.m_bank_details || '' },
      usdt:    { on: s.m_usdt_on === true, label: s.m_usdt_label || 'USDT',
                 rate: num(s.rate_usdt, 1), auto: false, details: s.m_usdt_address || '' },
    },
...
async function handleAdminSettings(user, body, env) {
  ...
  const allowedOps = [
    ...
    'm_bank_details', 'm_usdt_address', 'deposit_note',
    'theme_navy', 'theme_emerald', 'theme_bg', 'theme_radius',
    'nav_home', 'nav_issue', 'nav_cards', 'nav_wallet', 'nav_tx', 'nav_settings',
...
async function getSettings(env) {
  ...
  return {
    ...
    nav_wallet: true, nav_tx: true, nav_settings: true,
    // ── نصوص الواجهة ──
