...
async function handleStatus(env) {
  ...
  return {
    ...
    methods: {
      ...
      usdt:    { on: s.m_usdt_on === true, logo: s.m_usdt_logo || '', label: s.m_usdt_label || 'USDT',
...
async function handleSmsWebhook(request, env) {
  ...
  let matchUid = null;
  try {
    matchUid = await findUserByPhone(env, parsed.sender);
  } catch (e) {
    console.error('USER_LOOKUP_FAILED', e.message);
  }
  ...
  try {
    ...
    await Promise.all([
      ...
      fsSet(env, `wallet_deposits/${fingerprint}`, {
        ...
        amount_usd: amountUsd,
        amount_lyd: parsed.amount_lyd,
        method: (parsed.network === 'almadar' ? 'المدار' : 'ليبيانا') + ' — تلقائي',
        proof_url: '',
    ...
    console.error('AUTO_CREDIT_FAILED', e.message);
    await fsSet(env, `sms_transactions/${fingerprint}`, {
      ...record, status: 'unclaimed', needs_attention: true,
      error: String(e.message).slice(0, 200),
    }).catch(() => {});
