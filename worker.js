...
async function handleCatalog(env) {
  ...
  const products = prods
    ...
    .map(p => ({
      ...
      desc: p.desc || '',
      image: p.image || '',
      price: round2(num(p.price, 0)),          // السعر النافذ بالدينار
      old_price: round2(num(p.old_price, 0)),  // قبل الخصم — 0 يعني بلا خصم
      featured: p.featured === true,           // يظهر في «الأكثر طلبًا»
