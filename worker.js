...
// ═══════════════════════════════════════════════════════════
//  Phase 2 Imports (Multi-Provider Architecture)
// ═══════════════════════════════════════════════════════════
...
export default {
  ...
  /**
   * Cron trigger — يُضبط من wrangler.toml (مثلاً كل ساعة).
   * يمسح بيانات البطاقات المكشوفة منتهية الصلاحية فقط؛
   * لا يمس البطاقة نفسها ولا سجل الطلبات.
   */
...
async function route(path, request, url, env) {
  ...
  // ═══════════════════════════════════════════════════════════
  //  Phase 2: مسارات المزودين والمنتجات
  // ═══════════════════════════════════════════════════════════
...
/** خيارات BIN المتاحة للعميل — يفعّلها المسؤول من اللوحة */
const BIN_INFO = {
  '539502': { country: 'هونغ كونغ', flag: '🇭🇰', dob: false },
  '525847': { country: 'هونغ كونغ', flag: '🇭🇰', dob: false },
  '539578': { country: 'هونغ كونغ', flag: '🇭🇰', dob: false },
  '525797': { country: 'هونغ كونغ', flag: '🇭🇰', dob: false },
  '537872': { country: 'الولايات المتحدة', flag: '🇺🇸', dob: true },
  '533171': { country: 'سنغافورة', flag: '🇸🇬', dob: true },
  '246001': { country: 'المملكة المتحدة', flag: '🇬🇧', dob: true },
};

function binOptions(s) {
  const on = String(s.bins_enabled || BINS_NO_DOB.join(','))
    .split(',').map(x => x.trim()).filter(Boolean);
  return on
    .filter(b => BIN_INFO[b])
    .map(b => ({ bin: b, ...BIN_INFO[b] }));
}
...
async function handleStatus(env) {
  ...
  return {
    ...
    manual_cards: {
      on: s.manual_cards_enabled === true,
      create_min: num(s.mc_create_min, 10), create_max: num(s.mc_create_max, 500),
      create_fee_fixed: num(s.mc_create_fee_fixed, 0), create_fee_pct: num(s.mc_create_fee_pct, 0),
      topup_min: num(s.mc_topup_min, 10), topup_max: num(s.mc_topup_max, 500),
      topup_fee_fixed: num(s.mc_topup_fee_fixed, 0), topup_fee_pct: num(s.mc_topup_fee_pct, 0),
      tx_fee: num(s.mc_tx_fee, 0),
    },
    withdraw: {
...
async function handleCreateCard(user, body, env) {
  ...
  // العميل يختار بلد البطاقة؛ نرفض أي BIN خارج القائمة المعتمدة
  const ALL_BINS = [...BINS_NO_DOB, ...BINS_NEED_DOB];
  const asked = String(body.bin || '').trim();
  const bin = ALL_BINS.includes(asked) ? asked
    : String(s.default_bin || BINS_NO_DOB[0]);
  ...
  try {
    ...
    // 202 مع REFUND_PENDING: المبلغ محجوز لدى المزود ولم يُعَد.
    // نُعيد للعميل رصيده ونعلّم الطلب للمتابعة اليدوية، ولا نكرّر الطلب.
    if (err.refundPending) {
      await refundOrder(env, orderId, user.uid, p.customer_price,
        'محجوز لدى المزود — قيد المتابعة');
      await fsPatch(env, `card_orders/${orderId}`,
        { needs_attention: true, provider_hold: true }).catch(() => {});
      throw err;
    }
...
async function handleCatalog(env) {
  ...
  const categories = cats
    ...
    .map(c => ({
      id: c._id,
      name: c.name || '',
      parent: c.parent || '',          // '' = قسم جذر
      icon: c.icon || '',              // مفتاح من مكتبة الأيقونات
      image: c.image || '',
      soon: c.soon === true,
      sort: num(c.sort, 99),
    }));
  ...
  const products = prods
    ...
    .map(p => {
      ...
      return {
        ...
        desc: p.desc || '',
        image: p.image || '',
        // السعر النافذ: المحسوب تناسبيًا إن كان العداد مفعّلًا
        ...
        countdown: pr,        // null إن كان الزر مطفأً
...
/**
 * تسعير تناسبي للاشتراكات المشتركة.
 * المنتج له مدة محددة (مثلاً شهر)، والعميل ينضم في منتصفها،
 * فيدفع مقابل الأيام المتبقية فقط.
 *
 * يُحسب في الخادم دائمًا — لو تُرك للمتصفح لاستطاع أي شخص
 * تغيير ساعة جهازه ليشتري بسعر يوم واحد.
 */
function proration(p, nowMs) {
  ...
  // يوم الانتهاء محسوب ضمن المدة: من 1/9 إلى 30/9 = ثلاثون يومًا
  ...
  // لا ننزل تحت حد أدنى يحدده المسؤول — يحمي من بيع بسعر رمزي في آخر يوم
  const minP = round2(num(p.min_price, 0));
  // لا يتجاوز السعر الكامل — التقريب قد يرفعه فوقه في أول يوم
...
async function handleStoreOrder(user, body, env) {
  ...
  for (const it of items) {
    ...
    // السعر يُحسب هنا من جديد — لا نثق بأي رقم يرسله المتصفح
...
// ═══════════════════════════════════════════════════════════
//  السحب والتحويل بين المستخدمين
// ═══════════════════════════════════════════════════════════

/**
 * طلب سحب — يُخصم فورًا ويُحجز، ثم ينفّذه المسؤول أو يرفضه.
 * الخصم الفوري يمنع إنفاق نفس الرصيد مرتين أثناء انتظار المراجعة.
 */
async function handleWithdrawRequest(user, body, env) {
  ...
  if (amount < mn) throw httpError(400, `الحد الأدنى للسحب ${mn}$`);
  if (amount > mx) throw httpError(400, `الحد الأقصى للسحب ${mx}$`);
  ...
  // نخصم أولًا — الرصيد محجوز حتى تُنفَّذ أو تُرفض
  ...
  try {
    ...
    await fsIncrement(env, `users/${user.uid}`, { wallet_balance: amount }).catch(() => {});
    throw httpError(503, 'تعذّر إنشاء الطلب — أُعيد رصيدك');
  }
}

/** المسؤول ينفّذ السحب أو يرفضه فيُعاد المبلغ */
async function handleAdminWithdraw(user, body, env) {
  ...
  if (action === 'reject') {
    ...
    await Promise.all([
      ...
      logOp(env, user.uid, 'withdraw_rejected', { id }, { refunded: back }, true),
    ]);
    return { success: true, refunded: back };
  }
  ...
  await Promise.all([
    ...
    logOp(env, user.uid, 'withdraw_completed', { id }, {}, true),
  ]);
  return { success: true };
}

/** تحويل رصيد بين مستخدمين — فوري وذرّي */
async function handleTransfer(user, body, env) {
  const s = await getSettings(env);
  assertLive(s);
  if (s.transfer_enabled !== true) throw httpError(503, 'التحويل غير متاح حاليًا');

  const amount = round2(num(body.amount_usd, NaN));
  const to = String(body.to || '').trim().toLowerCase();
  if (!Number.isFinite(amount) || amount <= 0) throw httpError(400, 'أدخل المبلغ');
  if (!to) throw httpError(400, 'أدخل بريد أو رقم المستلم');

  const mn = num(s.transfer_min, 1);
  if (amount < mn) throw httpError(400, `الحد الأدنى للتحويل ${mn}$`);

  const feePct = num(s.transfer_fee_pct, 0);
  const fee = round2(amount * feePct / 100);
  const total = round2(amount + fee);

  // البحث عن المستلم بالبريد أو بالهاتف
  let target = null;
  if (to.includes('@')) {
    const rows = await fsQuery(env, {
  ...
  try {
    ...
    await fsIncrement(env, `users/${user.uid}`, { wallet_balance: total }).catch(() => {});
    throw httpError(503, 'تعذّر التحويل — أُعيد رصيدك');
  }
}
...
/**
 * يتحقق من كوبون ويُعيد قيمة الخصم.
 * لا يُستهلك هنا — الاستهلاك يتم داخل الطلب حتى لا يُحرق بلا شراء.
 */
async function checkCoupon(env, code, uid, subtotalLyd) {
  ...
  if (minOrder > 0 && subtotalLyd < minOrder) {
    throw httpError(400, `الكوبون يتطلب طلبًا بـ${minOrder} د.ل على الأقل`);
  }
...
/** يسجّل استهلاك الكوبون بعد نجاح الطلب فقط */
...
async function handleRedeemPoints(user, body, env) {
  ...
  const rateP = num(s.points_value_lyd, 0.01);   // دينار لكل نقطة
  ...
  await Promise.all([
    ...
    logOp(env, user.uid, 'redeem_points', { pts }, { usdVal }, true),
  ]);
...
// ═══════════════════════════════════════════════════════════
//  البطاقات اليدوية — تُنفَّذ عبر مزوّد خارجي (Cryptomus أو غيره)
//  وتُدخَل بياناتها يدويًا من لوحة الإدارة
// ═══════════════════════════════════════════════════════════

/**
 * طلب بطاقة جديدة أو إعادة شحن — يُخصم فورًا ويدخل قائمة الانتظار.
 * لا يتصل بأي مزوّد؛ المسؤول ينفّذه يدويًا ويُدخل البيانات لاحقًا.
 */
async function handleManualCardRequest(user, body, env) {
  const s = await getSettings(env);
  assertLive(s);
  if (s.manual_cards_enabled !== true) throw httpError(503, 'إصدار البطاقات متوقف مؤقتًا');

  const kind = body.kind === 'topup' ? 'topup' : 'create';
  const amount = round2(num(body.amount, NaN));
  if (!Number.isFinite(amount) || amount <= 0) throw httpError(400, 'أدخل المبلغ');

  const mn = num(kind === 'topup' ? s.mc_topup_min : s.mc_create_min, 10);
  const mx = num(kind === 'topup' ? s.mc_topup_max : s.mc_create_max, 500);
  if (amount < mn) throw httpError(400, `الحد الأدنى ${mn}$`);
  if (amount > mx) throw httpError(400, `الحد الأقصى ${mx}$`);

  let cardId = '';
  if (kind === 'topup') {
    cardId = String(body.card_id || '').trim();
  ...
  // ── الرسوم: ثابتة + نسبية، كلاهما يضبطهما المسؤول من اللوحة ──
  const fixedFee = kind === 'topup' ? num(s.mc_topup_fee_fixed, 0) : num(s.mc_create_fee_fixed, 0);
  const pct = kind === 'topup' ? num(s.mc_topup_fee_pct, 0) : num(s.mc_create_fee_pct, 0);
  const txFee = num(s.mc_tx_fee, 0);
  const fee = round2(fixedFee + amount * pct / 100 + txFee);
  const total = round2(amount + fee);
  ...
  try {
    await fsSet(env, `manual_card_orders/${id}`, {
      uid: user.uid, kind, amount, fee, total,
      card_id: cardId || '',
      card_name: String(body.card_name || '').slice(0, 40),
      name_on_card: String(body.name_on_card || '').slice(0, 40),
      status: 'pending',
      created_at: nowIso(), updated_at: nowIso(),
    });
    await logOp(env, user.uid, 'manual_card_request', { kind, amount }, { id }, true);
    return { success: true, id, fee, total };
  } catch (e) {
    await fsIncrement(env, `users/${user.uid}`, { wallet_balance: total }).catch(() => {});
    throw httpError(503, 'تعذّر إنشاء الطلب — أُعيد رصيدك');
  }
...
/**
 * المسؤول يُدخل بيانات البطاقة يدويًا بعد إصدارها عند المزوّد.
 * البيانات تُخزَّن مؤقتًا فقط لتسليمها للعميل، وتُمسح تلقائيًا —
 * لا تبقى أرقام بطاقات كاملة ولا CVV في قاعدة البيانات إلى الأبد.
 */
async function handleAdminCardFulfil(user, body, env) {
  ...
  if (body.action === 'reject') {
    await Promise.all([
      ...
      fsIncrement(env, `users/${o.uid}`, { wallet_balance: o.total }),
      logOp(env, user.uid, 'manual_card_rejected', { id }, { refunded: o.total }, true),
    ]);
    return { success: true, refunded: o.total };
  }
  ...
  if (!/^\d{2}\/\d{2}$/.test(expiry)) throw httpError(400, 'صيغة تاريخ الانتهاء MM/YY');
  ...
  if (o.kind === 'create') {
    ...
    await fsIncrement(env, `manual_cards/${cardId}`, { balance: o.amount }).catch(() => {});
  }

  await Promise.all([
    // بيانات كاملة تُحفظ مؤقتًا فقط — reveal_expires_at يحدد متى تُمسح
    fsSet(env, `manual_card_reveal/${id}`, {
      uid: o.uid, card_id: cardId,
      card_number: pan, expiry, cvv,
      created_at: nowIso(), expires_at: expiresAt,
    }),
    fsPatch(env, `manual_card_orders/${id}`, {
      status: 'completed', card_id: cardId, updated_at: nowIso(),
    }),
    logOp(env, user.uid, 'manual_card_fulfilled', { id }, { cardId }, true),
  ]);

  return { success: true, card_id: cardId, expires_at: expiresAt };
}

/** العميل يقرأ بيانات بطاقته مرة، ما دامت لم تُمسح بعد */
async function handleRevealCard(user, body, env) {
  ...
  return {
    success: true,
    card_number: rv.card_number, expiry: rv.expiry, cvv: rv.cvv,
    expires_at: rv.expires_at,
  };
}

/**
 * يُشغَّل دوريًا (Cron) لمسح بيانات البطاقات منتهية الصلاحية.
 * لا يمسح البطاقة نفسها — فقط رقمها الكامل وCVV، فيبقى last4
 * والسجل التاريخي بلا أي بيانات حساسة.
 */
...
/**
 * ينادي المزوّد ويطبّق عقد الردود الخاص به.
 *
 * الرد 202 مع pending هو الأخطر: إعادة المحاولة عليه هي المسار
 * الوحيد الذي قد يخصم من العميل مرتين. لذلك نميّزه بعلامة
 * `noRetry` ولا نعيد المحاولة أبدًا، ونفرّق بين حالتيه:
 *   بلا code            → أُعيد المبلغ تلقائيًا، لا شيء علينا
 *   REFUND_PENDING      → المبلغ محجوز ولم يُعَد، يحتاج متابعة يدوية
 */
async function kripi(path, payload, env) {
  ...
  // ── حد الاستخدام ──
  if (res.status === 429) {
    const wait = num(data.retry_after_seconds, 30);
    const e = httpError(429, `ضغط مؤقت على الخدمة — أعد المحاولة بعد ${wait} ثانية`);
    e.retryAfter = wait;
    e.noRetry = true;
    throw e;
  }

  // ── عملية سابقة ما زالت جارية ──
  if (data.code === 'OPERATION_IN_FLIGHT') {
    const e = httpError(409, 'هناك عملية جارية — انتظر حتى تنتهي');
    e.noRetry = true;
    throw e;
  }

  // ── 202: نتيجة غير مؤكدة، ممنوع تكرارها ──
  if (res.status === 202 || data.pending === true) {
    const refundPending = data.code === 'REFUND_PENDING';
    const e = httpError(202,
      refundPending
        ? 'تعذّر إتمام العملية والمبلغ محجوز لدى المزود — تواصل مع الدعم'
        : 'تعذّر تأكيد العملية ولم يُخصم منك شيء — حاول لاحقًا');
    e.pending = true;
    e.noRetry = true;
    e.refundPending = refundPending;
    throw e;
  }
...
async function getSettings(env) {
  ...
  return {
    ...
    store_enabled: true,
    subscriptions_visible: true,   // إخفاء قسم الاشتراكات من الواجهة بضغطة
    manual_cards_enabled: false,
    mc_create_min: 10, mc_create_max: 500,
    mc_create_fee_fixed: 0, mc_create_fee_pct: 0,
    mc_topup_min: 10, mc_topup_max: 500,
    mc_topup_fee_fixed: 0, mc_topup_fee_pct: 0,
    mc_tx_fee: 0,
    mc_reveal_hours: 24,
