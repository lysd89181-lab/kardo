...
async function route(path, request, url, env) {
  ...
  switch (path) {
    ...
    case '/api/telegram/code':      return handleTelegramCode(user, body, env);
    case '/api/referral/code':      return handleReferralCode(user, body, env);
    case '/api/referral/apply':     return handleReferralApply(user, body, env);
...
async function handleStatus(env) {
  ...
  return {
    ...
    methods: {
      ...
      binance: { on: s.m_binance_on === true, logo: s.m_binance_logo || '', label: s.m_binance_label || 'Binance Pay',
...
async function handleCreateCard(user, body, env) {
  ...
  await notify(env, user.uid,
    `✅ صدرت بطاقتك\nالرصيد: <b>$${round2(num(res.amount, amount))}</b>`
    + `\nتنتهي بـ ${res.last_4 || ''}`);
...
async function handleAdminSettings(user, body, env) {
  ...
  const allowedOps = [
    ...
    'notify_on', 'referral_on', 'referral_bonus', 'referral_min_spend',
    'daily_cards_max', 'daily_amount_max',
...
async function handleAdminDeposit(user, body, env) {
  ...
  if (action === 'reject') {
    ...
    await notify(env, dep.uid, `❌ رُفض إيداعك\nالسبب: ${why}`);
  ...
  await notify(env, dep.uid, `💰 اعتُمد إيداعك — أُضيف <b>$${amountUsd}</b> إلى محفظتك`);
...
async function handleWalletClaim(user, body, env) {
  ...
  if (sms) {
    ...
    await notify(env, user.uid, `💰 أُضيف <b>$${credited}</b> إلى محفظتك`);
...
/** يُعيد رمز إحالة العميل، ويُنشئه عند أول طلب */
async function handleReferralCode(user, body, env) {
  const s = await getSettings(env);
  ...
  if (!code) {
    code = randomSuffix(6);
    await Promise.all([
      fsPatch(env, `users/${user.uid}`, { referral_code: code }),
      fsSet(env, `referral_codes/${code}`, { uid: user.uid, created_at: nowIso() }),
    ]);
  }

  // من دُعُوا بهذا الرمز
  const invited = await fsQuery(env, {
    from: [{ collectionId: 'users' }],
    where: {
      fieldFilter: { field: { fieldPath: 'referred_by' }, op: 'EQUAL',
  ...
  return {
    success: true,
    enabled: s.referral_on === true,
    code,
    bonus: num(s.referral_bonus, 1),
    min_spend: num(s.referral_min_spend, 10),
    invited: invited.length,
    earned: num(u && u.referral_earned, 0),
  };
}

/** يربط العميل الجديد بمن دعاه */
async function handleReferralApply(user, body, env) {
  const s = await getSettings(env);
  if (s.referral_on !== true) throw httpError(503, 'نظام الدعوة غير مفعّل');

  const code = String(body.code || '').trim().toUpperCase();
  if (!code) throw httpError(400, 'أدخل رمز الدعوة');

  const u = await fsGet(env, `users/${user.uid}`);
  if (u && u.referred_by) throw httpError(400, 'استخدمت رمز دعوة من قبل');
  if (u && u.referral_code === code) throw httpError(400, 'لا يمكنك دعوة نفسك');
  if (num(u && u.total_spent, 0) > 0) {
    throw httpError(400, 'رمز الدعوة يُستخدم قبل أول عملية شراء');
  }

  const owner = await fsGet(env, `referral_codes/${code}`);
  if (!owner || !owner.uid) throw httpError(404, 'رمز غير صحيح');
  if (owner.uid === user.uid) throw httpError(400, 'لا يمكنك دعوة نفسك');

  await fsPatch(env, `users/${user.uid}`, { referred_by: code, referred_uid: owner.uid });

  return { success: true, min_spend: num(s.referral_min_spend, 10) };
}

/** يمنح المكافأة بعد أن يبلغ المدعوّ حدّ الإنفاق */
async function maybePayReferral(env, uid, s) {
  try {
    if (s.referral_on !== true) return;

    const u = await fsGet(env, `users/${uid}`);
    if (!u || !u.referred_uid || u.referral_paid === true) return;

    const need = num(s.referral_min_spend, 10);
    if (num(u.total_spent, 0) < need) return;

    const bonus = round2(num(s.referral_bonus, 1));
    if (bonus <= 0) return;

    await Promise.all([
      fsPatch(env, `users/${uid}`, { referral_paid: true }),
      fsIncrement(env, `users/${uid}`, { wallet_balance: bonus }),
      fsIncrement(env, `users/${u.referred_uid}`,
        { wallet_balance: bonus, referral_earned: bonus }),
    ]);

    await notify(env, uid, `🎁 وصلتك مكافأة الدعوة: <b>$${bonus}</b>`);
    await notify(env, u.referred_uid,
      `🎁 صديق دعوته أكمل أول عملية — ربحت <b>$${bonus}</b>`);
  } catch (e) {
    console.error('REFERRAL_PAY_FAILED', e.message);
  }
}
...
/**
 * يرسل إشعارًا للعميل عبر تليجرام.
 * يتطلب أن يكون العميل قد ربط حسابه (chat_id محفوظ لديه).
 * الفشل صامت — الإشعار لا يجوز أن يُسقط عملية مالية.
 */
async function notify(env, uid, text) {
  try {
    const token = env.TELEGRAM_BOT_TOKEN;
    if (!token) return;

    const s = await getSettings(env);
    if (s.notify_on !== true) return;

    const u = await fsGet(env, `users/${uid}`);
    const chatId = u && u.telegram_chat_id;
    if (!chatId) return;
    ...
    console.error('NOTIFY_FAILED', e.message);
  }
}

/** يستقبل تحديثات البوت لربط حساب العميل برمز مؤقت */
async function handleTelegramWebhook(request, env) {
  ...
  if (!msg || !msg.text) return { success: true, ignored: true };
  ...
  const reply = async (t) => {
    ...
    await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ chat_id: String(chatId), text: t, parse_mode: 'HTML' }),
    }).catch(() => {});
  };

  // /start CODE — الرمز يُولَّد من حساب العميل في الموقع
  const m = text.match(/^\/start\s+([A-Za-z0-9]{6,20})$/);
  if (!m) {
    await reply('أرسل الرمز الظاهر في صفحة الإعدادات داخل كاردو لربط حسابك.');
    return { success: true };
  }

  const code = m[1].toUpperCase();
  const link = await fsGet(env, `tg_links/${code}`);

  if (!link || !link.uid) {
    await reply('الرمز غير صالح أو منتهي. أنشئ رمزًا جديدًا من الإعدادات.');
    return { success: true };
  }
  if (num(link.expires_ms, 0) < Date.now()) {
    await reply('انتهت صلاحية الرمز. أنشئ رمزًا جديدًا من الإعدادات.');
    return { success: true };
  }

  await Promise.all([
    fsPatch(env, `users/${link.uid}`, { telegram_chat_id: String(chatId) }),
    fsPatch(env, `tg_links/${code}`, { used: true, used_at: nowIso() }),
  ]);

  await reply('تم ربط حسابك بكاردو ✅\nستصلك إشعارات الرصيد والبطاقات هنا.');
  return { success: true, linked: true };
}

/** يُنشئ رمز ربط مؤقت للعميل */
async function handleTelegramCode(user, body, env) {
  const code = randomSuffix(8);
  await fsSet(env, `tg_links/${code}`, {
    uid: user.uid,
    used: false,
    created_at: nowIso(),
    expires_ms: Date.now() + 15 * 60000,
  });
  return { success: true, code, bot: env.TELEGRAM_BOT_USERNAME || '' };
}
...
/**
 * يقرأ عدّاد اليوم لمستخدم (أو للمنصة إن كان uid = '_platform').
 * العدّادات مستندات مستقلة، فلا تحتاج استعلامات ولا فهارس.
 */
async function readCounter(env, uid) {
  const id = `${uid}_${todayKey()}`;
  const d = await fsGet(env, `daily_counters/${id}`);
  return {
    id,
 ...
 * kind: 'card' لإصدار بطاقة، 'deposit' لإضافة رصيد.
 */
...
async function handleUsdtVerify(user, body, env) {
  ...
  await notify(env, user.uid, `💰 وصل تحويل USDT — أُضيف <b>$${credited}</b> إلى محفظتك`);
...
async function handleSmsWebhook(request, env) {
  ...
  if (claim && claim.uid) {
    try {
      ...
      await notify(env, claim.uid, `💰 أُضيف <b>$${amountUsd}</b> إلى محفظتك`);
      return {
...
async function getSettings(env) {
  ...
  return {
    ...
    notify_on: false,
    // ── الإحالة ──
    referral_on: false, referral_bonus: 1, referral_min_spend: 10,
    // ── الحدود اليومية ──
