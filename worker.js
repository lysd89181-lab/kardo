...
const DEFAULT_SETTINGS = {
  ...
  support_url: '',
  deposit_mode: 'auto',
  plans_enabled: true, plan_free_cards: 1, plan_basic_cards: 3, plan_vip_cards: 10,
...
async function handleStatus(env) {
  ...
  return {
    ...
    plans: {
      ...
      basic: { cards: planCardLimit(s, 'basic'), price: num(s.plan_basic_price, 5), days: num(s.plan_basic_days, 30) },
      vip:   { cards: planCardLimit(s, 'vip'),   price: num(s.plan_vip_price, 15),  days: num(s.plan_vip_days, 30) },
    },
  };
}
...
function sanitizeSettingsPatch(body) {
  ...
  for (const k of ['plan_basic_days', 'plan_vip_days']) if (k in out && (out[k] < 1 || out[k] > 366)) throw httpError(400, 'مدة الباقة بين 1 و 366 يومًا');
  for (const k of ['plan_free_cards', 'plan_basic_cards', 'plan_vip_cards']) if (k in out && out[k] > 100) throw httpError(400, 'عدد بطاقات غير منطقي');
...
async function handleWalletClaim(user, body, env) {
  ...
  const manualMode = st.deposit_mode === 'manual';
...
async function handleManualCardRequest(user, body, env) {
  ...
  const resp = await fsRunTransaction(env, async (tx) => {
    ...
    const used = mine.map(r => withId(r, 'manual_card_orders')).filter(o => o.status !== 'rejected').length;
    if (used >= limit) throw httpError(403, `باقتك تسمح بـ ${limit} ${limit === 1 ? 'بطاقة' : 'بطاقات'} — رقِّ باقتك لإصدار المزيد`);
...
const PLAN_DEFAULT_CARDS = { free: 1, basic: 3, vip: 10 };
const PLAN_NAME_AR = { free: 'العادي', basic: 'الأساسية', vip: 'VIP' };

function userPlan(u, now = Date.now()) {
  const p = u && u.plan;
  if ((p === 'basic' || p === 'vip') && num(u.plan_expires_ms, 0) > now) return p;
  return 'free';
}
...
async function handlePlanSubscribe(user, body, env) {
  ...
  const plan = body.plan === 'vip' ? 'vip' : body.plan === 'basic' ? 'basic' : '';
  if (!plan) throw httpError(400, 'اختر الباقة');
  const key = idemKeyOf(body);
  const price = round2(num(s[`plan_${plan}_price`], 0));
  const days = Math.floor(num(s[`plan_${plan}_days`], 30));
  ...
  const resp = await fsRunTransaction(env, async (tx) => {
    ...
    if (cur === 'vip' && plan === 'basic') throw httpError(400, 'باقتك الحالية VIP — تُجدَّد بنفس الباقة أو تنتظر انتهاءها');

    let charge = price, credit = 0, base = now;
    if (cur === plan) base = num(L.doc.plan_expires_ms, now);                  // تجديد: يُضاف للمدة المتبقية
    if (cur === 'basic' && plan === 'vip') {                                    // ترقية: خصم قيمة المتبقي
      const bPrice = num(s.plan_basic_price, 0), bDays = Math.max(1, num(s.plan_basic_days, 30));
      credit = round2(Math.max(0, bPrice * (num(L.doc.plan_expires_ms, now) - now) / (bDays * 86400000)));
    ...
    ledgerClose(tx, L, { plan, plan_expires_ms: expires, plan_updated_at: nowIso() });
...
async function handleAdminUserPlan(user, body, env) {
  ...
  const uid = String(body.uid || '').trim();
  const plan = ['free', 'basic', 'vip'].includes(body.plan) ? body.plan : '';
  ...
  await fsRunTransaction(env, async (tx) => {
    const u = await tx.get(`users/${uid}`);
    if (!u) throw httpError(404, 'المستخدم غير موجود');
    tx.update(`users/${uid}`, { plan, plan_expires_ms: expires, plan_updated_at: nowIso(), plan_set_by: staff.uid });
  });
...
async function handleManualDeposit(user, body, env) {
  ...
  if (!proof) throw httpError(400, 'أرفق صورة الإيصال');
  ...
  await fsSet(env, `wallet_deposits/${id}`, {
    ...
    method: String(m.label || method).slice(0, 40), method_key: method, proof_url: proof, note,
...
async function handleAdminPhonePending(user, body, env) {
  await requirePermission(user, env, 'deposit.review');
  const rows = await fsQueryRaw(env, {
...
async function handleAdminCategorySave(user, body, env) {
  const staff = await requirePermission(user, env, 'store.manage');
  const id = String(body.id || '').trim();
...
async function sendEmail(env, to, subject, html) {
  ...
  try {
    ...
    if (!r.ok) { console.error('MAIL_FAILED', r.status); return false; }
    return true;
  } catch (e) { console.error('MAIL_FAILED', e.name); return false; }
}
...
async function handleAdminMailTest(user, body, env) {
  ...
  if (!ok) throw httpError(502, 'فشل الإرسال — تحقق من المفتاح وتوثيق النطاق في Resend');
  return { success: true };
}
