...
import { getFirestore, doc, getDoc, onSnapshot, collection, query,
  orderBy, limit }
...
function subscribe() {
  sub(query(collection(db, 'users'), orderBy('created_at', 'desc'), limit(100)), 'users');
  sub(query(collection(db, 'manual_cards'), orderBy('created_at', 'desc'), limit(100)), 'manualCards');
  sub(query(collection(db, 'manual_card_orders'), orderBy('created_at', 'desc'), limit(150)), 'manualOrders');
  sub(query(collection(db, 'orders'), orderBy('created_at', 'desc'), limit(100)), 'orders');
  sub(query(collection(db, 'wallet_deposits'), orderBy('created_at', 'desc'), limit(100)), 'deposits');
  sub(query(collection(db, 'withdrawals'), orderBy('created_at', 'desc'), limit(80)), 'withdrawals');
  sub(query(collection(db, 'tickets'), orderBy('updated_at', 'desc'), limit(60)), 'tickets');
  sub(query(collection(db, 'coupons'), limit(80)), 'coupons');
  sub(query(collection(db, 'sms_transactions'), orderBy('created_at', 'desc'), limit(80)), 'sms');
  sub(query(collection(db, 'services'), limit(150)), 'services');
  sub(query(collection(db, 'service_orders'), orderBy('created_at', 'desc'), limit(150)), 'serviceOrders');
...
function render() {
  ...
  setHTML('#view', (views[S.page] || vHome)());
  bind();
}
...
function vSms() {
  ...
  <div class="page-enter">
    <div class="mb-3">
      ...
      <h1 class="h2" style="margin-bottom:4px;font-family:var(--font-display)">رسائل التحويل</h1>
      <p class="body-sm text-2" style="font-size:11.5px">${S.sms.length} رسالة</p>
    </div>
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        ${S.sms.map(s => `
          <div class="card">
            <div class="flex-between mb-2">
              <div>
                <div class="tabular h4" style="font-size:14px;color:var(--sand)">${s.amount_lyd ? esc(lyd(s.amount_lyd)) : '—'}</div>
                <div class="caption mono" dir="ltr" style="font-size:10.5px">${esc(s.sender || s.from || '—')}</div>
              </div>
              <span class="badge ${s.status === 'claimed' ? 'badge-success' : s.status === 'untrusted' ? 'badge-error' : 'badge-warning'}">
                ${s.status === 'claimed' ? 'مربوطة' : s.status === 'untrusted' ? 'مزيّفة' : 'غير مربوطة'}
              </span>
            </div>
            <div class="caption mb-2" style="font-size:10.5px">${esc(dt(s.created_at))}</div>
            <div class="copy-box">
              <div class="copy-value" style="font-size:10.5px">${esc((s.raw_text || '').slice(0, 120))}</div>
            </div>
          </div>
        `).join('')}
      </div>
    ` : emptyState(I.sms, 'لا رسائل')}
  </div>
  `;
}
...
async function saveSettings(btn) {
  ...
  document.querySelectorAll('input[data-op], textarea[data-op]').forEach(el => {
...
function bind() {
  ...
  document.querySelectorAll('[data-settab]').forEach(b =>
    b.onclick = () => { S.setTab = b.dataset.settab; render(); });
